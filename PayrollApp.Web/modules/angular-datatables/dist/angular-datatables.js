...
(function(window, document, $, angular) {
    ...
    function dataTable($q, $http, DTRendererFactory, DTRendererService, DTPropertyUtil) {
        ...
        function compileDirective(tElm) {
            ...
            return function postLink($scope, $elem, iAttrs, ctrl) {
                function handleChanges(newVal, oldVal) {
                    if (newVal !== oldVal) {
                        ctrl.render($elem, ctrl.buildOptionsPromise($elem), _staticHTML);
                    }
                }
        ...
        function ControllerDirective($scope) {
            ...
            function buildOptionsPromise($elem) {
                var defer = $q.defer();
                // Build options
                $q.all([
                    ...
                    if (angular.isDefined(dtOptions)) {
                        ...
                        if (bindFilters) {
                            $elem.parent().find("div.toolbar").html('<div class="text-right"><img src="styles/img/logo.png" alt="SmartAdmin" style="width: 111px; margin-top: 3px; margin-right: 10px;"></div>');

                            $elem.on('keyup change', 'thead th input[type=text]', function () {
